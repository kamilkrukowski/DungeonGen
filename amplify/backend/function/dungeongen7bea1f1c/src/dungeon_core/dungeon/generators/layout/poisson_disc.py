...
class PoissonDiscLayoutGenerator(BaseLayoutAlgorithm):
    ...
    def _create_smart_connections(
        ...
        # Use k-NN with planarity pruning to create adjacency graph
        connections = self._create_knn_connections(rooms, guidelines)
    ...
    def _create_knn_connections(
        self, rooms: list[Room], guidelines: DungeonGuidelines
    ) -> list[Connection]:
        """
        Create connections by sampling ~30% of all possible edges.
        ...
        # Generate all possible room pairs
        all_pairs = []
        for i in range(len(rooms)):
            for j in range(i + 1, len(rooms)):
                ...
                if room1.anchor and room2.anchor:
                    ...
                    # Only consider reasonably close rooms
                    if distance <= 25.0:  # Maximum connection distance
                        all_pairs.append((room1, room2, distance))

        # Sample ~30% of all possible edges
        sample_rate = 0.3
        num_connections = max(1, int(len(all_pairs) * sample_rate))

        # Sort by distance (prefer shorter edges) and sample
        all_pairs.sort(key=lambda x: x[2])  # Sort by distance
        sampled_pairs = all_pairs[:num_connections]

        # Convert to connections
        connections = []
        for room1, room2, distance in sampled_pairs:
            _ = distance
            connections.append(
                Connection(
                    room_a_id=room1.id,
    ...
    def _create_fallback_connections(self, rooms: list[Room]) -> list[Connection]:
        """
        Create fallback connections for small numbers of rooms.
